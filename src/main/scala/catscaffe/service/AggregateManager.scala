...
import akka.actor._
import catscaffe.domain.AggregateRoot
...
trait AggregateManager extends Actor with ActorLogging {
  ...
  /**
   * Processes command.
   * In most cases it should transform message to appropriate aggregate command
   * (and apply some additional logic if needed) and call [[AggregateManager.processAggregateCommand]]
   *
   */
